...
import { deleteRestaurantUserByOwner } from "@/services/PostRequest/postRequest";
...
export default function Profile() {
  ...
  useEffect(() => {
    if (user) {
      fetchRestaurantOwnerInformation();
    }
    if (user == false) {
      route.push("/");
    }
  ...
  const handleDeleteAccount = async (e) => {
    ...
    if (userEmail == saitData[0].email) {
      await deleteRestaurantUserByOwner(auth.currentUser, saitData[0].id);
      route.push("/");
    }
  };
  return (
